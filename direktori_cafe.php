    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        bg60: '#140F0A', bg30: '#2C1E16', accent10: '#D4A373', textLight: '#FDF8F5', textMuted: '#A89B91'
                    }
                }
            }
        }
...
<body class="bg-bg60 text-textLight leading-relaxed antialiased min-h-screen flex flex-col">
                ...
                <div class="cafe-card bg-bg30 rounded-xl overflow-hidden border border-bg30 hover:border-accent10/50 transition-all duration-300 hover:-translate-y-1.5 flex flex-col"
                    ...
                    <div class="p-5 flex-1 flex flex-col">
                        ...
                        <div class="mt-auto flex gap-3">
                            <button onclick="window.location.href='detail_cafe.php?id=<?= $cafe['id'] ?>'" 
                                    class="flex-1 py-2 bg-transparent border border-accent10 text-accent10 font-bold text-sm rounded-md hover:bg-accent10 hover:text-bg60 transition-all">
                                Detail
                            </button>

                            <button onclick="window.location.href='detail_menu.php?id=<?= $cafe['id'] ?>'" 
                                    class="flex-1 py-2 bg-accent10 text-bg60 font-bold text-sm rounded-md hover:bg-[#c09161] transition-all">
                                Lihat Menu
                            </button>
                        </div>
                    </div>
    ...
    <div id="filterModal" class="fixed inset-0 z-[100] hidden bg-black/80 flex items-center justify-center p-4">
        <div class="bg-bg60 border border-accent10/30 p-6 rounded-xl max-w-sm w-full">
            <h3 class="text-xl font-bold mb-4 text-accent10">Pilih Fasilitas</h3>
            ...
            <button onclick="toggleModal(false)" class="w-full py-2 bg-accent10 text-bg60 font-bold rounded">Terapkan</button>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleModal(show) { document.getElementById('filterModal').classList.toggle('hidden', !show); }
        ...
        function applyFilters() {
            ...
            // Feedback tombol filter
            if(selectedFeatures.length > 0) {
                filterBtn.innerHTML = `<i class="fas fa-sliders-h"></i> Filter (${selectedFeatures.length})`;
                filterBtn.classList.add('border-accent10', 'text-accent10');
            } else {
                filterBtn.innerHTML = `<i class="fas fa-sliders-h"></i> Filter Lain`;
                filterBtn.classList.remove('border-accent10', 'text-accent10');
            }
            ...
            filteredCards = cards.filter(card => {
                ...
                return nama.includes(searchVal) && (locVal === "" || lokasi === locVal) && selectedFeatures.every(f => fitur.includes(f));
            });
        ...
        function renderGrid() {
            ...
            filteredCards.slice(startIdx, startIdx + itemsPerPage).forEach(card => card.style.display = 'flex');
